...
# P_MONI
email <-   s3read_using(FUN = arrow::read_parquet,
...
#        "Ronda - Lampada Acesa"
#)) %>% 
#  select(Protocolo = protocolo,Rua = endereco, Bairro = nome_bairro) %>% 
#  filter(!is.na(Rua),!is.na(Bairro))  
...
kbl(email,longtable = T,align = "c",escape = T) %>% 
